...
export class MCPConnection {
  ...
  async _sendInitializeRequest() {
    ...
    try {
      ...
      if (response.ok) {
        log(`[MCP ${this.name}] Initialize request successful (Status: ${response.status}). Proceeding with handshake...`);
        
        // Wait for a short period to let the server process the initialize request 
        // to prevent race conditions during the initial handshake
  ...
  async _sendInitializedNotification() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        const errorMsg = `Initialized notification failed: ${response.status} ${errorText}`;
        console.error(`[MCP ${this.name}] ${errorMsg}`, {
          endpoint: this.messageEndpoint.toString(),
          sessionId: this.sessionId
        });
        throw new Error(errorMsg);
      }
    } catch (error) {
      console.error(`[MCP ${this.name}] Initialized notification exception:`, error);
      throw error;
    }
  }
